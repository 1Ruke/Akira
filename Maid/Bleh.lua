...
--// Library init
	...
	function Library:CompareVars(Obj1, Obj2, ExpectedType)
		...
		local Type1, Type2, Expected = Typeof(Obj1), Typeof(Obj2), LowerString(ExpectedType);
	
		if LowerString(Type1) ~= Expected or LowerString(Type2) ~= Expected then
			print(FormatString('Type check failed: got %s and %s, expected %s', Type1, Type2, ExpectedType))
			warn(getgenv().debug.traceback())
		...
		function Library:EspPreview(Properties)
			...
			local Items = Cfg.Items; do
				...
				Library:Connection(RunService.RenderStepped, function()
					if not (Library.MenuIsOpen or Cfg or Cfg.Rotation) then return end;
					if (Flags['Menu Spin Preview']) then
						Cfg.Rotation += 0.75;
